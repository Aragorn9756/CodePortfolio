...
template <typename T>
void CourseStack<T>::push(const T& thing) {
	if (currentSize == capacity) {
		cout << "The stack is full. Nothing more may be added.";
		return;
	}
	else {
		stackPtr[currentSize] = thing;
		currentSize++;
	}
}
...
template <typename T>
void CourseQueue<T>::push_back(const T& thing) {
	//Full Queue
	if (capacity == currentSize) {
		cout << "The queue is full. Take your lawn chair elsewhere.";
		return;
	}
...
T CourseQueue<T>::pop_front() {
	...
	//queue is empty
	if (currentSize == 0) {
		cout << "There's nothing in the cue, so nothing to pop. Sorry.";
		throw (1);
	}

	tempVar = queuePtr[front];
	currentSize--;

	//front wraps around queue
	if (front == capacity - 1) {
		front = 0;
	}
	else {
